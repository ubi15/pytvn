...
def plpva(x,xmin, *varargin):
        ...
        if type(varargin[i])==str:
            if varargin[i]=='range':
                ...
                if Range[1]>Range[0]:
                    ...
                    vec=[]
                try:
                    vec = [X*float(Range[2])+Range[0] \
                            for X in range(int((Range[1]-Range[0])/Range[2]))]
    ...
    if f_dattype== 'REAL':
        ...
        cz    = map(lambda X:X/float(nz), xrange(0,nz))
        cf    = map(lambda X:1-pow((xmin/float(X)),(alpha-1)),z)
        gof   = max( map(lambda X:abs(cz[X] - cf[X]), xrange(0,nz)))
        pz    = nz/float(N)
        ...
        for B in range(0,Bt):
            ...
            for qm in xrange(0,len(qmins)):
                ...
                dat.append( max( map(lambda X: abs(cf[X]-float(X)/nq), xrange(0,nq))))
            if not quiet:
        ...
        xmax=max(z)
        L       = []
        for k in xrange(0,len(vec)):
            L.append(-vec[k]*float(sum(map(log,z))) - float(nz)*log(float(zvec[k]) - sum(map(lambda X:pow(float(X),-vec[k]), xrange(1,xmin)))))
        I = L.index(max(L))
        alpha=vec[I]
        fit = reduce(lambda X,Y: X+[Y+X[-1]],\
                     (map(lambda X: pow(X,-alpha)/(float(zvec[I])-sum(map(lambda X: pow(X,-alpha),map(float, xrange(1,xmin))))), xrange(xmin,xmax+1))),[0])[1:]
        cdi=[]
        for XM in xrange(xmin,xmax+1):
            cdi.append(len(filter(lambda X: floor(X)<=XM,z))/float(nz))
        
        gof= max( map(lambda X: abs(fit[X] - cdi[X]), xrange(0,xmax-xmin+1)))
        pz=nz/float(N)
        mmax = 20*xmax
        pdf=[]
        for i in xrange(0,xmin):
            pdf.append(0)
        pdf += map(lambda X: pow(X,-alpha)/(float(zvec[I])-sum(map(lambda X: pow(X,-alpha),map(float, xrange(1,xmin))))), xrange(xmin,mmax+1))
        ...
        for B in range(0,Bt):
            ...
            k=0
            q2=[]
            for i in xrange(xmin,mmax+2):
                while c<n2 and r2[c]<=cdf[i]:c+=1
                for k in xrange(k,c):q2.append(i)
                k=c
                if k>=n2: break
            ...
            for qm in xrange(0,len(qmins)):
                ...
                if nq>1:
                    ...
                    qminvec = xrange(1,qmin)
                    for k in xrange(1,len(vec)):
                        L.append(-vec[k]*float(slogzq) - float(nq)*log(float(zvec[k]) - sum(map(lambda X:pow(float(X),-vec[k]),qminvec))))
            
                    I = L.index(max(L))
                    fit = reduce(lambda X,Y: X+[Y+X[-1]],\
                                 (map(lambda X: pow(X,-vec[I])/(float(zvec[I])-sum(map(lambda X: pow(X,-vec[I]),map(float, xrange(1,qmin))))), xrange(qmin,qmax+1))),[0])[1:]
                    cdi=[]
                    for QM in xrange(qmin,qmax+1):
                        cdi.append(len(filter(lambda X: floor(X)<=QM,zq))/float(nq))
                    
                    dat.append(max( map(lambda X: abs(fit[X] - cdi[X]), xrange(0,qmax-qmin+1))))
